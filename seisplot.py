...
def main(target, cfg):
    ...
    Notice.info("data_shape   {}".format(data.shape))
    ...
    #####################################################################
    if cfg['plot']:
        Notice.hr_header("Plotting image")
    ...
    Notice.info("save numpy array   {}".format(cfg['save']))
    if cfg['save']:
        Notice.hr_header("Saving numpy array")
        t2 = time.time()
        s = "Saved image file {} in {:.1f} s"
        if cfg['outfile']:
            ...
            if os.path.isfile(cfg['outfile']):
                ...
                stem, ext = os.path.splitext(os.path.split(target)[1])
                outfile = os.path.join(cfg['outfile'], stem + '.npy')

            stem, _ = os.path.splitext(outfile)  # Needed for stupidity.
            np.save(outfile, data)
            t3 = time.time()
            Notice.ok(s.format(outfile, t3-t2))
        else:  # Do the default: save a npy in the same dir as the target.
            stem, _ = os.path.splitext(target)
            np.save(stem, data)
            t3 = time.time()
            Notice.ok(s.format(stem+'.npy', t3-t2))
        return
